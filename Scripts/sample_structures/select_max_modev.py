...
top_n = sys.argv[1]  # Specify the number of top structures to extract (e.g., top 100)
...
with open(input_file, 'r') as f:
    ...
    for idx, line in enumerate(lines):
        ...
        if max_force_deviation > sys.argv[2]:
...
print(f'Number of filtered structures: {len(filtered_deviations)}')
...
# Output the indices of the top N max force deviations (from the filtered list)
print(f'Indices of the top {top_n} max force deviations (from the filtered list): {top_indices_in_filtered}')
