...
Purpose:    Perform NEB calculations with a NEP model using NepTrainKit.
Usage:      python neb_calculation_neptrain.py <initial_structure> <final_structure>
            <n_image> <nep_model>
Arguments:
...
if len(sys.argv) != 5 or sys.argv[1] in ("-h", "--help"):
    print(" Usage: python neb_calculation_neptrain.py <initial_structure> <final_structure> <n_image> <nep_model>")
    print("        (NepTrainKit variant of neb_calculation.py)")
    print("")
    print(" Arguments:")
    print("   initial_structure  Path to the initial structure XYZ file")
    print("   final_structure    Path to the final structure XYZ file")
    print("   n_image            Number of intermediate images (e.g., 9)")
    print("   nep_model          Path to the NEP model file")
    print("")
    print(" Example: python neb_calculation_neptrain.py init.xyz final.xyz 9 nep.txt")
    print("")
...
def print_dependency_notice():
    print(" This function requires the NepTrainKit package.")
...
for input_file in (initial_file, final_file, nep_model):
...
if len(images) > 0:
    while fix_method not in ['none', 'index', 'element', 'position']:
        ...
        fix_method = read_input(" Enter fixing method (none/index/element/position): ").strip().lower()
    ...
    if fix_method == 'index':
        while True:
            try:
                indices_input = read_input(" Enter atom indices to fix (0-based, space-separated): ").strip()
        ...
        while True:
            fixed_element = read_input(" Enter element symbol to fix (e.g., Mg): ").strip()
        ...
        while True:
            try:
                range_input = read_input(" Enter x_min x_max y_min y_max z_min z_max: ").strip()
