...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Calculate displacement from trajectory/model and neighbor list.",
        formatter_class=HelpFormatter,
        epilog=(
            "Examples :\n"
            "1. Single-frame model.xyz:\n"
            ">>  gpumdkit.sh -calc disp -i model.xyz -n nl-Pb-O.dat -o displacements.dat\n"
            ">>  python calc_displacement.py -i model.xyz -n nl-Pb-O.dat -o displacements.dat\n"
            "2. Multi-frame movie.xyz with slice:\n"
            ">>  python calc_displacement.py -i movie.xyz -n nl-Pb-O.dat -s 100 -t 500 -p 2 -o disp_slice.dat\n"
            "3. Use the last 20% frames:\n"
            ">>  python calc_displacement.py -i movie.xyz -n nl-Pb-O.dat -l 0.2 -o disp_last20.dat"
        ),
    )
    ...
    parser.add_argument(
        ...
        "--neighbor-list",
        required=True,
        help="Neighbor list file.",
    )
    parser.add_argument("-o", "--output", default="displacements.dat", help="Output text file.")
...
def save_displacements(file_name, displacements):
    save_data = displacements
    if displacements.ndim > 2:
        save_data = displacements.reshape(-1, 3)
    np.savetxt(file_name, save_data)
    print(f"Saved displacement data to {file_name}, shape={save_data.shape}.")
...
def main():
    ...
    displacements = calculate_displacement(traj, nl, select=select)
    save_displacements(args.output, displacements)
