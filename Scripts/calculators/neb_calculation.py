...
if len(images) > 0:
    ...
    if fix_method == 'index':
        while True:
            try:
                indices_input = read_input(" Enter atom indices to fix (0-based, space-separated, e.g., '0 1 2'): ").strip()
        ...
        while True:
            try:
                range_input = read_input(" Enter x_min x_max y_min y_max z_min z_max (space-separated floats): ").strip()
