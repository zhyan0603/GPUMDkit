<div align="center">
  <h1>📊 Plot Scripts</h1>
    <p style="text-align: justify;">This directory contains Python scripts for comprehensive visualization of data generated by <strong>GPUMD</strong> and <strong>NEP</strong>. These plotting tools help analyze molecular dynamics simulations, NEP training results, and various material properties.</p>
</div>
...
For XRD output generated by calculator 413, use `gpumdkit.sh -plt xrd`.
...
The command reports the fitted activation energies and diffusivities. With
`save`, it writes `Arrhenius_D_PT.png`.
...
The command reports the fitted activation energies and conductivity values.
With `save`, it writes `Arrhenius_sigma_PT.png`.
...
<div align="center">
    <img src="../../docs/Gallery/vac_pdos.png" alt="VAS PDOS" width="70%" />
</div>
...
`column_index` uses the `rdf.out` output-column numbering: column 2 is the
total RDF and columns 3 and above are pair RDFs.

---

#### plt_xrd.py

Plots the X-ray diffraction (XRD) curve generated by `calc_xrd.py`.

**Input File:** `xrd.out` by default, or a file path supplied on the command line
...
The saved image is `xrd_comp.png` in the current working directory.
...
| `pdos`         | `model.xyz`, `run.in`, `dos.out`, `mvac.out` | Phonon density of states           |
