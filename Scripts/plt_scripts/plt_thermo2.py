...
=============================================================================
Script:     plt_thermo2.py
Category:   Plot Scripts
Purpose:    Alternative thermodynamic property visualization from thermo.out
            with different layout and style.
Usage:      gpumdkit.sh -plt thermo2 [save]
            python plt_thermo2.py [save]
Arguments:
...
# Read number of atoms from model.xyz
# Read number of atoms from model.xyz and check for replicate command in run.in
def get_num_atoms():
    # Step 1: Read base number of atoms from model.xyz
    if os.path.exists('model.xyz'):
        with open('model.xyz', 'r') as file:
            base_num_atoms = int(file.readline().strip())  # First line contains the number of atoms
    else:
        raise FileNotFoundError("model.xyz file not found.")

    # Step 2: Check for replicate command in run.in
    replication_factor = 1  # Default: no replication
    if os.path.exists('run.in'):
        with open('run.in', 'r') as file:
            for line in file:
                if "replicate" in line:
                    try:
                        # Split the line and get the replication factors (e.g., "replicate 2 2 2")
                        parts = line.split()
                        if len(parts) >= 4:  # Ensure there are at least 3 numbers after "replicate"
                            x, y, z = int(parts[1]), int(parts[2]), int(parts[3])
                            replication_factor = x * y * z  # Total replication factor
                        break
                    except (IndexError, ValueError):
                        # If parsing fails, ignore and use default replication_factor
                        pass

    # Step 3: Calculate final number of atoms
    total_num_atoms = base_num_atoms * replication_factor
...
# Get number of atoms and calculate PE per atom
...
pe_min, pe_max = np.min(potential_energy_per_atom), np.max(potential_energy_per_atom)
pe_range = pe_max - pe_min
pe_ylim_lower = pe_min - 0.95 * pe_range  # Double the range downward
pe_ylim_upper = pe_max + 0.05 * pe_range  # Double the range upward
axs[0, 2].set_title(r'$P_E$ per Atom vs $K_E$')
axs[0, 2].set_xlabel('Time (ps)')
axs[0, 2].set_ylabel(r'Potential Energy per Atom (eV)', color='tab:orange')
axs[0, 2].plot(time, potential_energy_per_atom, color='tab:orange')
...
ke_ylim_upper = ke_max + 0.95 * ke_range  # Double the range upward
