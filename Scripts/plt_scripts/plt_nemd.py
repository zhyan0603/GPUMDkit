...
Purpose:    Post-processing for NEMD (Non-Equilibrium Molecular Dynamics)
            ...
            thermostat energy, optional SHC spectral analysis, and data export.
Usage:      gpumdkit.sh -plt nemd [real_length] [scale_eff_size] [cutoff_freq] [--save] [--save-data]
...
Arguments:
  ...
  --save-data     Also export 'data_nemd.txt' and, when SHC is present,
                  'data_shc.txt'
Output:
  nemd.png    (if --save is used)
  data_nemd.npz (legacy output; always written)
  data_shc.npz  (legacy output when SHC data exist)
  data_*.txt    (if --save-data is used)
...
def print_usage():
    ...
    print("Usage: gpumdkit.sh -plt nemd [real_length] [scale_eff_size] [cutoff_freq] [--save] [--save-data]")
    print("Params:")
    print("  real_length   : Real length of heat transfer zone in nm (set to 'Auto', with auto-calculation)")
    ...
    print("  --save-data   : Optional, additionally export 'data_nemd.txt' and, when SHC data exist,")
    print("                  'data_shc.txt' (independent of --save; can be used on its own)")
    print("                  Historical .npz outputs are written as before.")
    print("  Legacy bare 'save' and 'save_data' are also accepted.")
    print("  --save/--save-data may appear anywhere on the command line, e.g.:")
    print("    gpumdkit.sh -plt nemd --save-data          (defaults for everything else)")
    print("    gpumdkit.sh -plt nemd Auto 1 60 --save --save-data")
...
class NEMD_Processor:
    def __init__(self, _directory, _real_length=None, _scale_eff_size=1, _cutoff_freq=60, _scale_vacuum=1,
        ...
        save_data : bool
            Also save the processed data as .txt files; the historical .npz
            outputs remain enabled for NEMD.
        """
    ...
    def process_SHC(self, deltaT):
        ...
        # Keep the historical NEMD .npz output; --save-data adds the text export.
    ...
    def _export_txt(self, Reformed_NEMD_data, N_nemd_data, N_temp_group, xticks_length, Time_upper, Length):
        ...
        with open(os.path.join(self.directory, 'data_nemd.txt'), 'w', encoding='utf-8') as f:
            f.write("\n".join(lines) + "\n")
    ...
    def _export_shc_txt(self, Reformed_SHC_data):
        ...
        with open(os.path.join(self.directory, 'data_shc.txt'), 'w', encoding='utf-8') as f:
            f.write("\n".join(lines) + "\n")
