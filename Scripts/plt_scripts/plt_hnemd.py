...
Purpose:    Post-processing for HNEMD (Homogeneous Non-Equilibrium Molecular
            ...
            average, optional SHC spectral analysis, and data export.
Usage:      gpumdkit.sh -plt hnemd [scale_eff_size] [cutoff_freq] [--save] [--save-data]
...
Arguments:
  ...
  --save-data     Save processed data as 'data_hnemd.npz'/'data_hnemd.txt';
                  when SHC is present, also save 'data_shc.npz'/'data_shc.txt'
Output:
  hnemd.png    (if --save is used)
  data_*.npz   (if --save-data is used)
...
def print_usage():
    ...
    print("Usage: gpumdkit.sh -plt hnemd [scale_eff_size] [cutoff_freq] [--save] [--save-data]")
    print("Params:")
    ...
    print("    gpumdkit.sh -plt hnemd --save-data          (defaults for everything else)")
    print("    gpumdkit.sh -plt hnemd 1 60 --save --save-data")
...
class HNEMD_Processor:
    ...
    def _export_txt(self, Reformed_HNEMD_data, t):
        ...
        with open(os.path.join(self.directory, 'data_hnemd.txt'), 'w', encoding='utf-8') as f:
            f.write("\n".join(lines) + "\n")
