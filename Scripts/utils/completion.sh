...
_gpumdkit_completions() {
    ...
    local opts="-h -help -update -U -clean -time -plt -calc -cbc -range -out2xyz -out2exyz -cp2k2xyz -pos2exyz -cif2pos -cif2exyz -exyz2pos -xdat2exyz -pos2lmp -lmp2exyz -traj2exyz -addgroup -addlabel -addweight -min_dist -min_dist_pbc -filter_dist -filter_dist_pbc -filter_box -filter_value -filter_range -get_frame -clean_xyz -get_volume -analyze_comp -chem_species -replicate -hbond -pda -pynep -frame_range -re_atoms -nep_modifier"
    ...
    case "$prev" in
        ...
        -calc)
            COMPREPLY=($(compgen -W "ionic-cond nep des doas neb minimize msd nlist disp avg-struct oct-tilt pol-abo3" -- "$cur")) ;;
            
        # Options requiring files or directories, complete with filenames
        -out2xyz|-out2exyz|-cp2k2xyz|-exyz2pos|-min_dist|-min_dist_pbc|-filter_dist|-filter_dist_pbc|-filter_box|-filter_value|-filter_range|-get_frame|-clean_xyz|-pos2exyz|-cif2exyz|-cif2pos|-pos2lmp|-lmp2exyz|-traj2exyz|-addgroup|-addlabel|-addweight|-analyze_comp|-replicate|-pda|-cbc|-frame_range|-chem_species|-xdat2exyz|-re_atoms)
