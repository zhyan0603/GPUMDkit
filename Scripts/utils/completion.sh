...
_gpumdkit_completions() {
    ...
    case "$prev" in
        ...
        -plt)
            COMPREPLY=($(compgen -W "thermo thermo2 thermo3 train train_density prediction test train_test parity_density born_charge bec msd msd_all msd_conv msd_sdc sdc rdf rdf_pmf vac restart dimer force_errors des doas charge lr arrhenius_d arrhenius_sigma sigma D sigma_xyz D_xyz net_force emd emd2 nemd hnemd pdos phonon phonon_comp plane-grid cohesive viscosity" -- "$cur")) ;;
