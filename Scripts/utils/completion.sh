...
_gpumdkit_completions() {
    ...
    local opts="-h -update -help -clean -time -plt -calc -cbc -range -out2xyz -xml2xyz -pos2exyz -cif2pos -cif2exyz -exyz2pos -pos2lmp -lmp2exyz -addgroup -addlabel -addweight -min_dist -min_dist_pbc -filter_dist -filter_dist_pbc -filter_box -filter_value -get_frame -clean_xyz -get_volume -analyze_comp -replicate -hbond -pda -pynep"
    ...
    case "$prev" in
        ...
        -plt)
            COMPREPLY=($(compgen -W "thermo thermo2 thermo3 train prediction test train_test msd msd_all msd_conv sdc rdf vac restart dimer force_error des doas charge lr parity_density arrhenius_d arrhenius_sigma sigma D net_force emd nemd hnemd pdos" -- "$cur")) ;;
