...
安装后，在准备存放转换结果的工作目录中运行 `gpumdkit.sh`。相对路径以终端当前目录为起点；输出位置见各命令说明，并不一定与输入文件同目录。建议先复制少量数据到新的练习目录，避免覆盖已有结果。

本页优先介绍已安装版本可用的命令和菜单。`python Scripts/...` 是可选的源码调用方式：需要在 GPUMDkit 源码根目录运行，并相应调整输入路径；也可以在数据目录中使用脚本的绝对路径。仅通过 Conda 安装时，优先使用 `gpumdkit.sh`，不必切换到安装目录。
...
| `dp2xyz` | `dp2xyz.py` | DeepMD npy 转 extxyz | 为 GPUMD/NEP 流程转换 DeepMD 数据集 |
| `xyz2dp` | `xyz2dp.py` | extxyz 转 DeepMD npy | 从带标签的 extxyz 准备 DeepMD 数据集 |
...
| 包含 BEC 数据的 OUTCAR 目录 | extxyz | `gpumdkit.sh -out2xyz_bec <dir>` |
...
| DeepMD npy 目录 | extxyz | `gpumdkit.sh -dp2xyz database train.xyz` |
| extxyz | DeepMD npy 数据集 | `gpumdkit.sh -xyz2dp` 或直接运行 Python 脚本 |
| extxyz | clean extxyz | `gpumdkit.sh -clean_xyz input.xyz clean.xyz` |

## 看懂输入和输出文件名

第一次转换时，可以先使用只有两个参数的 POSCAR 路线，文件流向最直观：
...
`POSCAR` 是输入结构文件名，`output.xyz` 是命令创建的 extxyz 文件。通配符由 shell 展开；批量转换前应先确认实际匹配到的文件名。转换完成后可以先做一个轻量格式检查：

```bash
head -n 2 output.xyz
```

第一行是原子数，第二行包含晶胞和已有性质等 extxyz 元数据。这本身不能验证计算的物理合理性；用于训练或模拟前，请再按分析工具教程检查数据集。

## 常见例子

### 将 DeepMD 数据集转换为 extxyz

使用 `-dp2xyz` 递归扫描 DeepMD 数据集目录，并写出 extxyz 文件：

```bash
gpumdkit.sh -dp2xyz database train.xyz
```

输入目录必须包含 `type.raw`、`type_map.raw` 和 `set.000/` 等 DeepMD 数据集文件。该命令依赖 `dpdata` 和 `ase`。

### 将带标签的 extxyz 转换为 DeepMD 数据集

使用直接转换入口，然后输入源文件和明确的 DeepMD type-map 顺序：

```bash
gpumdkit.sh -xyz2dp
```

源码用户也可以直接运行 Python 脚本。下面假定终端当前目录是 GPUMDkit 源码根目录；请将 `train.xyz` 换成数据文件的实际路径：

```bash
python3 Scripts/format_conversion/xyz2dp.py train.xyz Li P S
```

输入应包含带能量和力标签的 extxyz 数据。输出写入当前目录下的 `deepmd_data/`。该命令依赖 `dpdata`。
...
上面是 shell 版本。也可以选择 Python 版本；同一批数据只需选择其中一种：

```bash
gpumdkit.sh -out2exyz ./vasp_results/
```

对于包含完整 VASP `BORN EFFECTIVE CHARGES` 数据块的 OUTCAR 文件，可以使用
带 BEC 的 shell 转换器：
...
该入口为每个 OUTCAR 写出一个最终构型；当 BEC 数据块完整时，追加逐原子的
`bec:R:9` 属性。没有 BEC 数据块的 OUTCAR 仍会转换，但对应原子行不包含该属性，
同时会显示警告。
...
**输出位置：** 两种命令都从指定的 `vasp_results/` 读取数据，但将结果写入**终端当前目录**：

| 入口 | 输出文件 |
|------|----------|
| `-out2xyz` 或菜单 `101` | `NEPdataset/train.xyz` |
| `-out2xyz_bec` | `NEPdataset/train.xyz` |
| `-out2exyz` | `train.xyz` |

!!! warning "重复转换前检查输出目录"
    shell 版本和 BEC shell 版本会删除并重建当前目录下已有的 `NEPdataset/`；Python 版本会覆盖当前目录下的 `train.xyz`。请先备份结果，或在新的工作目录中运行并指定输入目录的实际路径。

转换后可用 `head -n 2 NEPdataset/train.xyz`（shell/BEC 版本）或 `head -n 2 train.xyz`（Python 版本）查看原子数和 extxyz 元数据。文件生成成功不等于数据可直接用于训练，还需检查计算收敛情况、单位和标签。
...
会将 extxyz 中所有帧转换成当前目录下的 `POSCAR_1.vasp`、`POSCAR_2.vasp`
等文件。程序按照输入轨迹中元素首次出现的顺序对每帧原子分组；例如
...
从 extxyz 轨迹中提取第 `1000` 帧（帧编号从 1 开始，第一帧为 `1`）。
...
生成 `model_1.xyz`、`model_2.xyz`、... 等单帧文件。
