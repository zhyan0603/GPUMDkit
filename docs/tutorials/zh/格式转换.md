...
| 入口 | 功能 | 适用场景 |
|------|------|----------|
| `101` | VASP OUTCAR 转 extxyz，shell 版本 | 批量转换 VASP 计算目录 |
| `102` | MTP cfg 转 extxyz | 转换 MTP 数据 |
| `103` | CP2K 转 extxyz | 可选择 log/inp 路线或 pos/frc/cell 路线 |
| `104` | ABACUS 转 extxyz | 转换 ABACUS SCF 或 MD 输出 |
| `105` | extxyz 转 POSCAR | 将 extxyz 每一帧写成 POSCAR |
| `106` | 添加 group 标签 | 为 GPUMD 相关流程添加原子分组标签 |
| `107` | 添加权重 | 修改训练集结构权重 |
| `108` | 提取单帧 | 从 extxyz 轨迹导出某一帧 |
| `109` | 清理 XYZ 信息 | 删除多余 extxyz 属性 |
| `110` | 扩胞 | 按倍数或目标原子数扩胞 |
| `out2exyz` | OUTCAR 转 extxyz，Python 版本 | VASP OUTCAR 转换的另一个入口 |
| `pos2exyz` | POSCAR 转 extxyz | 转换单个结构 |
| `cif2pos` | CIF 转 POSCAR | 从 CIF 准备 VASP 输入 |
| `cif2exyz` | CIF 转 extxyz | 从 CIF 准备 GPUMDkit 输入 |
| `xdat2exyz` | XDATCAR 转 extxyz | 转换 VASP MD 轨迹 |
| `pos2lmp` | POSCAR 转 LAMMPS data | 准备 LAMMPS 输入 |
| `lmp2exyz` | LAMMPS dump 转 extxyz | 转换 LAMMPS 轨迹 |
| `traj2exyz` | ASE traj 转 extxyz | 转换 ASE 轨迹 |
...
一些 GPUMD 相关流程会使用 group 标签进行原子分组，例如特定元素的 MSD 或扩散性质计算。
