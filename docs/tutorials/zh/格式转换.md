...
!!! warning "重复转换前检查输出目录"
    shell 版本会删除并重建当前目录下已有的 `NEPdataset/`；Python 版本会覆盖当前目录下的 `train.xyz`。请先备份结果，或在新的工作目录中运行并指定输入目录的实际路径。

转换后可用 `head -n 2 NEPdataset/train.xyz`（shell 版本）或 `head -n 2 train.xyz`（Python 版本）查看原子数和 extxyz 元数据。文件生成成功不等于数据可直接用于训练，还需检查计算收敛情况、单位和标签。
