...
如果你刚开始使用 GPUMDkit，请先阅读[快速入门](快速入门.md)；需要让 Agent
协助处理任务时，请先看 [GPUMDkit Agent Skill](模拟与后处理.md)；远程网页访问请参阅
[远程 Web 控制台](远程网页控制台.md)。只有提供了
对应帮助接口的命令才支持 `-h`；其他命令请使用 `gpumdkit.sh -h`、相应模块的
帮助或交互模式。
...
`-calc nep` 会生成包含 NEP 预测性质的 extxyz 输出文件；`-prediction` 则会
写出训练和验证流程使用的独立 NEP 预测文件。
...
| **Export model package** | 使用统一且不冲突的文件后缀写出模型、可选 restart、更新后的输入及来源摘要。 |
