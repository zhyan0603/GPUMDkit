<div align="center">
  <h1>📦 结构采样</h1>
  <p style="text-align: justify;">结构采样用于为 NEP 训练和分子动力学流程选择、生成或筛选候选构型。</p>
</div>

**脚本位置：** `Scripts/sample_structures/`

在候选轨迹、参考数据和模型所在的工作目录中运行 `gpumdkit.sh`，按菜单提示输入文件名；文件位于其他目录时，请输入实际路径。相对路径以终端当前目录为起点，生成文件也会写入各功能说明的位置，运行前请检查是否已有同名结果。

下文的 `python Scripts/...` 仅作为可选源码调用示例：需在 GPUMDkit 源码根目录运行并调整数据路径，或在数据目录中使用脚本的绝对路径。Conda 用户可直接使用菜单，不必寻找 `Scripts/` 目录。
...
`perturb.txt` 提供简洁的总体统计，而不是逐一列出每个生成结构。报告
给出晶胞长度、角度和体积变化的最小值、平均值和最大值，扣除晶胞仿射
变形后的非仿射原子位移 ASCII 分布，以及收缩、膨胀、晶胞应变和原子
位移最显著的结构。安装 ASE 时还会给出周期性最短距离统计。这些数据
仅用于描述生成结构；GPUMDkit 不会自行应用结构合格阈值。

该功能需要 `dpdata`。使用此功能时建议引用 dpdata 包。

## 最大力偏差结构选择

该功能需要配合 GPUMD 的 `active` 命令使用。`active` 采用委员会模型（committee）机制：多个势函数对同一结构预测力，并记录每个结构的最大力偏差。`select_max_modev.py` 会从 `active.out` 和 `active.xyz` 中选择力偏差较大的结构，写入 `selected.xyz`。
...
`-filter_box` 将保留的结构写入 `filtered_by_box.xyz`。随后在菜单 `203` 的参数提示处输入：

```text
filtered_by_box.xyz train.xyz nep.txt 4
```

这四项分别是过滤后的候选文件、已有参考数据集、兼容的 NEP 模型和并行核数。先准备好 `train.xyz` 和 `nep.txt`，再运行 FPS；没有执行过滤时，候选文件仍应使用原轨迹。示例中的盒长阈值 `13` 需要根据自己的体系确定，不能直接作为通用筛选标准。
