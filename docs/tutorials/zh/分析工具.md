...
| 能量参考平移 | `gpumdkit.sh -shift_energy` | 交互式平移 extxyz 能量参考 |
| 最短距离 | `gpumdkit.sh -min_dist dump.xyz` | 快速距离检查（无 PBC） |
| 最短距离（PBC） | `gpumdkit.sh -min_dist_pbc dump.xyz` | 考虑周期边界的距离检查 |
...
| 概率密度 | `gpumdkit.sh -pda <参考结构> <轨迹> <元素> <网格间距>` | 用于扩散通道分析的 3D 概率密度 |
| GPUMD 时间 | `gpumdkit.sh -time gpumd` | 估算 GPUMD 剩余运行时间 |
| NEP 时间 | `gpumdkit.sh -time nep` | 估算 NEP 剩余训练时间 |

## 过滤前先选择检查方式

在可能的情况下，应先运行不修改数据的检查命令，再运行过滤命令。这样可以先在终端中看到排除结构的理由。
...
也可以打开 GPUMDkit，选择 `5) Analyzer -> 509`。页面会先显示检测到的
元素和 `config_type` 分组，然后提供三种模式：

1. 参考组对齐：将选定分组对齐到参考组的平均能量。
2. 零原子基线对齐：为每个分组优化原子能量基线，使平移后的能量目标为零。
3. DFT 到 NEP 对齐：计算 NEP 能量，并将原始 DFT 能量对齐到 NEP 能量。

脚本保留原有 NES 默认参数；如有需要，可以在 Python 交互页面中修改高级
NES 设置。DFT 到 NEP 模式需要 `calorine` 和兼容的 NEP 模型文件。
