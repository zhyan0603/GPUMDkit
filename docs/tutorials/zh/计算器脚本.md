...
使用此功能可以用训练好的 NEP 模型进行预测。建议在使用前先验证模型在目标结构上的预测质量。
...
选项 `406–410` 涵盖钙钛矿和极性材料分析：近邻列表、局域位移、平均结构、八面体倾转和 ABO3 极化。这些工具基于 `ferrodispcalc`。
...
完整参数列表和真实工作流案例（PbTiO3 相变、PTO/STO 超晶格拓扑等）请参见 **[极性材料分析](极性材料分析.md)**。
