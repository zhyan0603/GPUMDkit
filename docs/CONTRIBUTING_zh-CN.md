...
- **`src/`**：交互模式菜单函数的 Shell 脚本
  ...
  - `f6_plots.sh`
  - `f7_utilities.sh`
- **`skills/`**：可移植的 Agent Skills 定义（`gpumdkit-skill/SKILL.md`）及按需加载的 references
- **`docs/`**：文档文件
...
2. **实现你的功能**，明确参数要求。Python 脚本负责详细参数解析、帮助信息、
   类型转换、文件检查和面向用户的报错：
   ```python
...
3. **在 `gpumdkit.sh` 中添加命令行标志处理**。这一层应保持为轻量路由，
   详细参数检查放在 Python 脚本中，不在主 shell 入口里重复实现。
   ```bash
...
- **Shell 脚本**：
  - 使用 `${variable}` 进行变量展开
  - 使用 `[ ! -z "$var" ]` 处理 shell 控制流和大模块分发检查（项目约定）
  - 交互函数应遵循 banner 格式：
    ```bash
    echo " >-------------------------------------------------<"
    ...
    echo " >-------------------------------------------------<"
    ```
  - 使用 `read_menu_choice varname || return 1` 或 `read_menu_array varname || return 1` 读取交互输入，然后用 `"${varname[@]}"` 传递数组
...
- **Python 脚本**：
  - 编写清晰、可维护的代码
  - 使用有意义的变量名
  - 简单位置参数使用 `args = sys.argv[1:]`，复杂选项组合使用 `argparse`
  - 提供清晰的使用信息，并保留前导空格，例如 `print(" Usage: ...")` 和 `print(" Example: ...")`
  - 将帮助和缺参检查放在重型可选依赖导入之前，确保未安装可选依赖时 `-h` 仍可使用
