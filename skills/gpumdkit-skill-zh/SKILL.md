...
- 在生成或翻译 GPUMD `run.in` 之前，先确认 GPUMD 版本。对于 GPUMD 5.7 及更高版本，原先由已删除 dump 关键字承担的输出功能使用 `dump_xyz`；`dump_position`、`dump_velocity`、`dump_force` 和 `dump_exyz` 只能作为 `references/gpumd-outputs.md` 中明确标注的 5.7 之前迁移语法。
