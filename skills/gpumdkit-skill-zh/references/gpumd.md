...
本技能中的 GPUMD 文件构成了 GPUMD 输入输出规范的分类摘要，内容自成体系。它们定义了支持的关键字族、当前参数格式、默认值、约束、生命周期和预期输出，无需依赖 GPUMD 源码树。
...
| 势函数加载、初始化、模拟盒变更、力、约束、最小化、运行 | `references/gpumd-setup.md` |
...
5. 将结构优化、平衡和正式采样明确分段。必要时在每个 `run` 段重新给出不会自动延续的系综、计算、dump 和控制命令。
6. 显式计算物理时长和输出频率：
...
- 本快照遵循 GPUMD 5.8.1 的输出语法。在生成或翻译 `run.in` 之前，先确认用户使用的 GPUMD 版本。GPUMD 5.7 及更高版本的原子级输出只使用 `dump_xyz`；已删除的 5.7 之前关键字集中记录在 `gpumd-outputs.md` 的迁移说明中。
