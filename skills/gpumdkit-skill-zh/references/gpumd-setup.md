...
使用此自包含目录了解非系综设置命令、外部控制、最小化和 `run`。高级物理参数仍需要用户明确决定。
...
| `compute_extrapolation` | `compute_extrapolation asi_file <file> gamma_low <v> gamma_high <v> check_interval <n> dump_interval <n>` | ASI 监控。默认值：`gamma_low=0`，实际上无上限的 `gamma_high`，两个间隔均为 1；在生产中依赖这些值之前需要用户批准 |
...
| 关键字 | 当前签名 | 重要含义/约束 |
|---|---|---|
| `change_box` | `change_box <delta>`；`change_box <delta_xx> <delta_yy> <delta_zz>`；或 6 参数形式后跟 `<epsilon_yz> <epsilon_xz> <epsilon_xy>` | 即时盒子变更。长度增量单位为 Angstrom；剪切项无量纲；6 参数形式需要三斜盒子 |
...
| `add_spring` | `add_spring ghost_com|ghost_atom|com_com ... couple|decouple ...` | 使用以下六个完整签名之一；弹簧常数和参考值由用户提供 |
...
| `mc vcsgc` | `mc vcsgc <md_steps> <mc_trials> <T_i> <T_f> <num_species> {<species> <phi> ...} kappa [group ...]` | 方差约束 SGC；`phi` 和 `kappa` 需要定义的热力学协议 |

## 即时操作

| 关键字 | 当前签名 | 重要含义/约束 |
|---|---|---|
| `minimize` | `minimize <sd|fire> <force_tolerance> <max_steps> [<box_change> [<hydrostatic_strain>]]` | 容差单位为 eV/Angstrom；盒子优化目前需要 FIRE |
...
这些签名随技能附带。如果本地解析器拒绝某个签名，请捕获可执行文件版本和确切错误，然后询问匹配的版本行为。在选择任何物理值之前，无论解析器是否接受，都要询问用户。
