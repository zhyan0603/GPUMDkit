...
使用此自包含参考查找计算的签名、输出、兼容性规则和参数角色。选择物理或收敛参数前请询问用户。
...
`compute_phonon` 需要 `kpoints.in`；当需要复制时，将 `replicate` 放在 `run.in` 的开头，然后在此即时计算之前加载势函数。位移单位为 Angstrom。当前签名没有 cutoff 参数。
...
- `compute_hac` 在 EMD 生产轨迹中运行，通常是在平衡后进行 NVE。
...
- 计算关键字通常是运行作用域/非传播的；为每个预期的生产块重新发出它们。
- 确保 `Nc * sample_interval` 明显短于生产运行时长，并在适用时使用保存的窗口/副本验证收敛性。
- 重新运行前确认输出追加行为。
