...
- 结构、势函数、压力/体积条件和相稳定性；
- 系综序列、时间步长、平衡时长、正式采样时长和重复次数；
- MSD 采样间隔/相关长度或轨迹帧间隔；
- 拟合接受标准、不确定度方法，以及是否允许外推（例如到 300 K）。

不要从文件夹名、化学式或常见做法中推断这些信息。如果用户只说"做 Arrhenius 计算"，在创建正式计算输入之前先问这些问题。
...
除非研究设计另有说明，否则在各温度之间使用相同的组成和经用户确认且保持一致的方案。记录种子和重复次数。两个有效温度是直线的数学最低要求，但科学证据较弱；请用户确认充分的设计，而不是自己选择数量。
...
从 `run.in` 和实际轨迹节奏确认此值。当帧的输出频率较低时，不要传递积分时间步长。
