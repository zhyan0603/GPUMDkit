...
使用此自成体系参考选择正确的命令族并解释其参数。系综选择和耦合值是科学决策；请询问用户。
...
`T_1` 和 `T_2` 是初始/最终目标温度，单位为 K，在运行期间线性渐变。`T_coup` 以时间步单位控制恒温器耦合。不要在没有用户确认的方案或有来源支持的方法的情况下选择它。
...
压力和弹性模量单位为 GPa。弹性常数是粗略的转换参数，Agent 不得自行编造。超过 2000 GPa 的分量会在文档中的标准压力控制器中禁用该模拟盒分量的耦合。
...
MTTK 方向包括 `iso`、`aniso`、`tri`，以及单独的 `x`、`y`、`z`、`xy`、`yz`、`xz` 分量。文档中 MTTK 的默认周期为温度 100 个时间步、压力 1000 个时间步；压力周期应至少为 200 个时间步。将这些视为文档默认值，而非可直接采用的正式计算参数。QTB `f_max` 是最大色噪声频率，单位为 ps^-1（默认 200，应大于系统最高声子频率）；`N_f` 是频率点数（默认 100，滤波器点数为 `2*N_f`）。QTB 压力周期必须至少为 200 个时间步。
...
- 询问模拟盒形状/体积是否需要固定、各向同性、各向异性或完全三斜。
- 压力控制前检查 PBC 和模拟盒形状限制。
- 询问温度/压力渐变是否是有意的。
- 将系综命令放在它们控制的同一 `run` 块中；它们不传播。
- 验证达到的温度/压力和模拟盒行为，而不仅仅是解析器成功。
