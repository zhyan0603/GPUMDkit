...
新增菜单项后，必须实际运行 `gpumdkit.sh` 并进入对应模块，检查终端中最终显示的子菜单。每一行右侧的 `|` 都必须与相邻行对齐。不能只数上下边框中的横线：内容行末尾多一个或少一个空格，同样会使右边框错位。还应比较新菜单行与相邻行的字符数，并完整走一遍新功能，检查 Shell 包装函数与 Python 脚本衔接后的整体交互效果。
...
- 所有辅助函数都有文档字符串
- 鼓励新脚本使用 `if __name__ == "__main__":` 入口保护
...
- 所有内容行的显示宽度必须与边框及相邻行一致，尤其要检查右侧 `|` 前的补齐空格
- 每新增一个菜单项，都要运行 `gpumdkit.sh`、进入上级模块并目视检查真实终端输出
- 至少完整进入一次新功能，把 Shell 与 Python 的提示作为一个连续界面审查
...
修改交互菜单时，只有通过 `gpumdkit.sh` 打开对应子菜单并确认所有右边框对齐后，验证才算完成。

### 同步维护英文和中文技能

`skills/gpumdkit-skill/` 是技术内容的英文基准。当其中的行为、路由、命令、默认值、约束或文件覆盖范围发生变化时，必须在同一次修改中更新 `skills/gpumdkit-skill-zh/` 下的对应文件。

- 命令、文件名、关键字、单位、公式、代码块、链接、表格、警告和数值必须与英文保持一致，除非底层功能本身也发生了变化。
- 翻译应传达原意，而不是照搬英文语序。采用自然、规范的中文技术术语；`dump_xyz`、`ensemble`、`type`、`compute_msd` 等程序关键字仍使用代码格式保留原文。
- 两棵 reference 目录必须逐文件对应。中文可以为便于理解增加简短说明，但不得遗漏英文中的安全规则、依赖、支持选项或新增功能。
- 翻译完成后，对照检查标题、表格、代码围栏和相对链接，并搜索遗漏的英文正文和生硬直译。
